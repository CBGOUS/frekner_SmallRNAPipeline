...
/**
 * Convert FASTQ to FASTA format using the FASTX_toolkit from CSHL
 * 
 * @author joey
 */
public class StepConvertFastqToFasta extends NGSStep{
    
    static  Logger                      logger = LogManager.getLogger();

    public static final NGSStepSubclass STEP_SUBCLASS           = NGSStepSubclass.DATABLE;

    public static final String          STEP_ID_STRING          = "ConvertFastqToFasta";
    ...
    public StepConvertFastqToFasta(){
     ...
     * 
     */
    public StepConvertFastqToFasta(InputDataForStep sid){
        classSubtype = NGSStepSubclass.DATABLE;
        stepInputData = sid;
    }
    ...
    public String shortStepDescription(){
      return "Convert FASTQ to FASTA format using the FASTX_toolkit from CSHL";
    }
    ...
    public String longStepDescription(){
      return "Convert FASTQ to FASTA format using the FASTX_toolkit from CSHL.\n"
              + "The step requires the FASTX_toolkit software to be installed.\n"
...
////        while (itSD.hasNext()){
//        SampleDataEntry sampleData = (SampleDataEntry) this.getStepInputData().getSampleData().get(0);
//        
...
//        
//        if (inFolder.contains("//")){
//            inFolder = "/" + inFolder.split("//")[1];
//        }
//        
//        if (new File(inFolder).isFile()){
//            fastqInputFile = inFolder.replace(sampleData.getFastqFile1Suffix(), INFILE_EXTENSION);
//        }
//        else {
//            fastqInputFile = this.cleanPath(inFolder + FILESEPARATOR + thisRunningFileName.replace(sampleData.getFastqFile1Suffix(), INFILE_EXTENSION));
//        }
//        fastqInputFile = fastqInputFile.replace("fastq_files", "result_files");
//        
//        if (new File(outFolder).isFile()){
//            fastaOutputFile = outFolder.substring(0, outFolder.length()-3);
//            fastaOutputFile = fastaOutputFile.replace("fastq_files", "result_files");
//        }
//        else{
//            fastaOutputFile = outFolder + FILESEPARATOR + thisRunningFileName.replace(sampleData.getFastqFile1Suffix(), FASTA_OUTFILE_EXTENSION); 
////            logger.info("sampleData.getFastqFile1().replace(sampleData.getFastqFile1Suffix(), FASTA_OUTFILE_EXTENSION)" + sampleData.getFastqFile1().replace(sampleData.getFastqFile1Suffix(), FASTA_OUTFILE_EXTENSION));
//            fastaOutputFile= fastaOutputFile.replace("fastq_files", "result_files");
//        }
//        
//        
...
//            cmdQ2A.add(this.getFastq2fasta_software());
//
////            fastqInputFile = this.cleanPath(inFolder + FILESEPARATOR + sampleData.getFastqFile1().replace(sampleData.getFastqFile1Suffix(), INFILE_EXTENSION));
//            cmdQ2A.add("-i");
//            cmdQ2A.add(fastqInputFile);
//
//            cmdQ2A.add("-o");
//
////            fastaOutputFile = outFolder + FILESEPARATOR + sampleData.getFastqFile1().replace(sampleData.getFastqFile1Suffix(), FASTA_OUTFILE_EXTENSION);
//            cmdQ2A.add(fastaOutputFile);
//
//            cmdQ2A.add("-Q33");
//
...
//            throw new IOException(STEP_ID_STRING + ": no Fastq1 file specified");
//        }
////        String fastqFile1 = inFolder + NGSStep.FILESEPARATOR + sampleData.getFastqFile1().replace(sampleData.getFastqFile1Suffix(), INFILE_EXTENSION);
//        inFolder = "/" + inFolder.split("//")[1];        
//        String fastqFile1 ="";
//        if (new File(inFolder).isFile()) {
//            fastqFile1 =inFolder.replace(sampleData.getFastqFile1Suffix(), INFILE_EXTENSION);
//        }
//        else{
//            fastqFile1 = inFolder + NGSStep.FILESEPARATOR + thisRunningFileName.replace(sampleData.getFastqFile1Suffix(), INFILE_EXTENSION);
//        }
//        
//        fastqFile1 = fastqFile1.replace("fastq_files", "result_files");
//        
//        if ((new File(fastqFile1)).exists() == false) {
//            logger.error(STEP_ID_STRING + ": fastq File1 <"
//                    + fastqFile1 + "> does not exist");
...
//            String fastqFile2 = inFolder + NGSStep.FILESEPARATOR + thisRunningFileName.replace(sampleData.getFastqFile1Suffix(), INFILE_EXTENSION);
//
//            if ((new File(fastqFile2)).exists() == false) {
//                logger.error(STEP_ID_STRING + " : fastq File2 <"
//                        + fastqFile2 + "> does not exist");
